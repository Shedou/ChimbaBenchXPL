...
func select_list_fill():
	get_node(btn_select_list).add_item("Complex", 0)
	get_node(btn_select_list).add_item("Windows", 1)
	get_node(btn_select_list).add_item("Linux", 2)
...
func _on_Select_List_item_selected( ID ):
	if ID == 0: selected_platform = "Complex"
	elif ID == 1: selected_platform = "Windows"
	elif ID == 2: selected_platform = "Linux"
	list_fill(selected_scene, selected_platform)
